...
import Placeholder from '../../images/user/photo-placeholder.jpeg';

export interface RowPhotoProps {photoURL: string; desc: string;}

export const RowPhoto: React.FC<RowPhotoProps> = ({photoURL, desc}) => (
  <div className="h-7 w-7 rounded">
    <img src={photoURL || Placeholder} alt={desc} />
  </div>
)

export interface ColSortProps {onSort: (id: string) => void; id: string;}

export const ColSort: React.FC<ColSortProps> = ({id, onSort}) => (
  <div onClick={() => onSort(id)} className="inline-flex flex-col space-y-[2px] absolute pl-2 pt-1.5 cursor-pointer">
    <span className="inline-block"><svg className="fill-current" width="10" height="5" viewBox="0 0 10 5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 0L0 5H10L5 0Z" fill=""></path></svg></span>
    <span className="inline-block"><svg className="fill-current" width="10" height="5" viewBox="0 0 10 5" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M5 5L10 0L-4.37114e-07 8.74228e-07L5 5Z" fill=""></path></svg></span>
  </div>
)
...
export const SearchTable: React.FC<SearchTableProps> = ({ name, placeholder, onChange, onSubmit, value, searchProps }) => (
  <div className="sm:block">
          <form onSubmit={(e) => {
            ...
            <div className="relative">
              <button type="submit" title="search" className="absolute top-1/2 left-0 -translate-y-1/2">
                <svg
...
export interface HeaderViewItemProps {children: React.ReactNode; isRight?: boolean; classNames?: string;}

export const HeaderViewItem: React.FC<HeaderViewItemProps> = ({children, isRight, classNames}) => (
...
export const TableRow: React.FC<PropsChildrenOnly> = ({ children }) => (
  <thead>
    <tr className='text-left dark:bg-meta-4  border-b border-[#eee] dark:border-strokedark'>
      {children}
    </tr>
  </thead>
)
...
//<th>item</th>
export const Row: React.FC<RowProps> = ({ row, children, rowIndex, isLastItem, rowWidth }) => {
  if (rowIndex === 0) {
    return (<th className={`min-w-[${rowWidth || '220px'}] py-4 px-4 font-medium text-black dark:text-white xl:pl-11`}>
      {row || children}
    </th>)
  }

  if (isLastItem) {
    return (<th className='py-4 px-4 font-medium text-black dark:text-white'>
      {row || children}
    </th>)
  }

  return (
    <th className={`'min-w-[${rowWidth || '120px'}] py-4 px-4 font-medium text-black dark:text-white`}>
      {row || children}
    </th>
  );
}
...
//<td>item</td>
export const Cell: React.FC<CellProps> = ({ cell, children, cellIndex, noBorder, isLink, onNavigate }) => {
  let val = (
    <td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
      <p className='text-black dark:text-white'>{cell || children}</p>
    </td>);

  if (cellIndex === 0) {
    val = (
      <td onClick={(e) => e.stopPropagation()} className='border-b border-[#eee] py-5 px-4 dark:border-strokedark xl:pl-11'>
        <p className='text-black dark:text-white'>{cell || children}</p>
      </td>
    )
  }
    ...
    val = (
      <td className='border-b border-[#eee] dark:border-strokedark py-5 px-4'>
        <div className='flex items-center space-x-3.5'>
          <button title="onNavigateButton" onClick={onNavigate} className='hover:text-primary ml-5'>
            <svg
...
export const CellRows: React.FC<CellRowsProps> = ({ children, useSelectOption, onClick}) => {
...
export const RowCheckInput: React.FC<RowCheckInputProps> = ({isChecked, onChecked, id}) => (
...
//<td>item</td>
export const StatusCell: React.FC<StatusCellProps> = ({ cell, children, variant }) => {
  let val = <p className='inline-flex rounded-full bg-white bg-opacity-10 py-1 px-3 text-sm font-medium text-black'>{cell || children}</p>

  if (variant === 'success') {
    val = <p className='inline-flex rounded-full bg-success bg-opacity-10 py-1 px-3 text-sm font-medium text-success'>{cell || children}</p>
  }

  if (variant === 'error') {
    val = <p className='inline-flex rounded-full bg-danger bg-opacity-10 py-1 px-3 text-sm font-medium text-danger'>{cell || children}</p>
  }

  if (variant === 'warning') {
    val = <p className='inline-flex rounded-full bg-warning bg-opacity-10 py-1 px-3 text-sm font-medium text-warning'>{cell || children}</p>
  }

  if (variant === 'primary') {
    val = <p className='inline-flex rounded-full bg-primary bg-opacity-10 py-1 px-3 text-sm font-medium text-primary'>{cell || children}</p>
  }

  return (<td className='border-b border-[#eee] py-5 px-4 dark:border-strokedark'>
    {val}
  </td>)
}
            ...
            <div className="col-span-12 flex flex-wrap items-center justify-end gap-3">
                    {
                      totalPages && activePage ? (
                        <div className='datatable-info'>
                          <p><span>{activePage}</span> of <span>{totalPages}</span></p>
                        </div>
                      ) : null
                    }
                    <nav>
                    ...
                    <div className="relative z-20 inline-block rounded bg-white shadow-card-2 dark:bg-boxdark">
                        <select title="pageLimit" value={activeLimit} onChange={e => onChangeLimit(e.target.value)} name="pageLimit" id="pageLimit" className="relative z-20 inline-flex appearance-none rounded border border-stroke bg-transparent py-2 pl-4 pr-9 font-medium text-sm outline-none dark:border-strokedark">
                            {pageLimit?.map((pl: number, pli: number) => <option key={pl+'-'+pli} value={pl}>{pl} / Page</option>)}
                        </select>
